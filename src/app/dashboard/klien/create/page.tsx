...
export default function CreateClientPage() {
  ...
  // Removed consultation step states
  ...
  // Removed job and profiling data state
  ...
  const handleOCR = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}/api/ocr/ktp`, {
        method: 'POST',
        body: formDataObj,
      });
  ...
  const handleNPWPOCR = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const backendUrl = process.env.NEXT_PUBLIC_BACKEND_API_URL || "http://localhost:3001";
      const response = await fetch(`${backendUrl}/api/ocr/npwp`, {
        method: 'POST',
        body: formDataObj,
      });
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 mt-1 font-medium">
               Gunakan fitur OCR untuk ekstraksi data otomatis dari KTP.
            </p>
          </div>
        </div>
      </div>
           ...
           <div className="animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100 flex items-center justify-between mt-auto">
               ...
               <div className="flex gap-4">
                 <Button onClick={handleSave} className="bg-primary hover:bg-primary/90 font-bold px-10 h-11 rounded-xl shadow-lg shadow-primary/20 gap-2"><CheckCircle2 className="h-4 w-4" /> Simpan & Daftarkan Klien</Button>
               </div>
            </div>
           </div>
      </div>
